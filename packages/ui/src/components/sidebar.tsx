	...
	if (collapsible === "none") {
		return (
			<div
				data-slot="sidebar"
				className={cn(
					"glass-sidebar flex h-full w-(--sidebar-width) flex-col text-sidebar-foreground",
					className,
				)}
				{...props}
			>
		...
		return (
			<Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
				<SheetContent
					...
					data-slot="sidebar"
					data-mobile="true"
					className="glass-sidebar w-(--sidebar-width) border-border/70 p-0 text-sidebar-foreground [&>button]:hidden"
					style={
						{
		...
		>
			...
			>
				<div
					...
					className="glass-sidebar flex size-full flex-col group-data-[variant=floating]:rounded-lg"
				>
					{children}
				</div>
			</div>
...
function SidebarTrigger({
	...
	return (
		...
		>
			<span
				aria-hidden="true"
				className="relative block size-4 border border-current/70"
			>
				<span className="absolute inset-y-0 left-1 w-px bg-current/70" />
			</span>
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
	...
	return (
		<button
			...
			className={cn(
				"absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear after:absolute after:inset-y-0 after:start-1/2 after:w-px hover:after:bg-border/70 group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
				"in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize",
				"[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
				"group-data-[collapsible=offcanvas]:translate-x-0 hover:group-data-[collapsible=offcanvas]:bg-transparent group-data-[collapsible=offcanvas]:after:left-full",
...
function SidebarInput({
	...
	return (
		<Input
			...
			data-sidebar="input"
			className={cn(
				"h-8 w-full border-border/70 bg-transparent shadow-none",
				className,
			)}
			{...props}
		/>
...
function SidebarSeparator({
	...
	return (
		<Separator
			...
			className={cn("mx-2 w-auto bg-border/70", className)}
			{...props}
		/>
	);
}
...
const sidebarMenuButtonVariants = cva(
	"peer/menu-button group/menu-button relative flex min-h-11 w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-foreground/5 hover:text-foreground focus-visible:ring-2 active:bg-foreground/5 active:text-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-foreground/5 data-active:font-medium data-active:text-foreground data-open:hover:bg-foreground/5 data-open:hover:text-foreground group-data-[collapsible=icon]:size-11! group-data-[collapsible=icon]:p-0! [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
	{
		variants: {
			variant: {
